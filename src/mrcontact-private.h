...
#define MR_ORIGIN_SECUREJOIN_INVITED    0x1000000 /* set on Alice's side for contact like Bob that have scanned the QR code offered by her. Only means the contact has once been established using the "securejoin" procedure in the past, for current key verification status check mrcontact_is_verfied(). */
#define MR_ORIGIN_SECUREJOIN_JOINED     0x2000000 /* set on Bob's side for contacts scanned and verified from a QR code. Only means the contact has once been established using the "securejoin" procedure in the past, for current key verification status check mrcontact_is_verfied(). */
