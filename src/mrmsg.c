		...
		if( pathNfilename ) {
			if( strncmp(mailbox->m_blobdir, pathNfilename, strlen(mailbox->m_blobdir))==0 )
			{
				...
				if( !file_used_by_other_msgs ) {
					...
					if( msg->m_type==MR_MSG_VOICE ) {
						char* extra_file = mr_mprintf("%s.waveform", pathNfilename);
						mr_delete_file(extra_file); /* the .waveform files may be generated by the frontends */
						free(extra_file);

						extra_file = mr_mprintf("%s.increation", pathNfilename);
						mr_delete_file(extra_file); /* the .increation files may be used to delay sending until eg. a video is created */
						free(extra_file);
					}
				}
			}
