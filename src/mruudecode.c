...
char* mruudecode_do(const char* text, char** ret_binary, size_t* ret_binary_bytes, char** ret_filename)
{
	...
	mr_unify_lineends(txt);

	// find again in new buffer
	uustartpos = mr_find_uuencoded_part(txt);
	if (uustartpos){
		// then handle uuencoded part
		ret_msg_text = mr_handle_uuencoded_part (txt, uustartpos, ret_binary, ret_binary_bytes, ret_filename);
	}
	free(txt);
	
	#ifdef __DEBUG__
	  puts("printing now remaining message after mr_handle_uuencoded_part() call\n\n");
	  gets(in);
	  puts("go ...\n\n");
	#endif
 ...
 *
 *  This is version 4 of the mruudecode.c
 * 	It the api which is used in  mrmimeparser.c  of the original project
...
int mr_getline (char** ret_lineptr, char* source, char** ret_nextcharptr){
	/**
	...
	if(line && (nread > 0)){
		...
		if(0 < mode && mode <= 777 && len_filename){
			...
			*ret_filename = strdup(filename);
			...
			#ifdef __DEBUG__
			printf("mr_handle_uuencoded_part - this is not an uu_part, stopping here!\n");
			#endif
...
int mr_uudecode(char** ret_binary, size_t uudecoding_buffer_len, const char* uu_body_start){
	...
	while( (line_len = mr_getline (&line, nextlinestart, &nextlinestart)) ){
		...
		if (!mr_uu_check_line(n, line_len - 1)){
